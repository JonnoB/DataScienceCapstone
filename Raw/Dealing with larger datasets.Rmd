...
```{r, eval=FALSE}
#save(ngram_probs, file ="ngram_probs.Rda")
load("ngram_probs_with_stops.Rda")
ngram_probs2 <-ngram_probs
load("ngram_probs.Rda")

ngram_probs <- ngram_probs2
```


testing the Ngram model
```{r} 
pattern <- "When you breathe, I want to be the air for"
...
result <- function(phrase, ngram_probs) { 
  phrase <- str_trim(phrase, side = "left")
